...
message_content = ""  # Per memorizzare il contenuto del messaggio JSON
...
def draw_info_panel():
    ...
    # Mostra file jason
    info_display.setFont("Arial", 8, True)
    info_display.drawText(f"JSON: {message_content}", 10, 85)
    info_display.setFont("Arial", 14, True)
...
def load_fsa_message(percorso_file, last_modified_time):
    global first_load, message_content, FSA  # Add FSA to global declarations
    
    # Store the current FSA in a local variable to ensure we always have something to return
    current_fsa = FSA
    
    # Controlla se il file è stato modificato
    current_modified_time = is_file_modified(percorso_file, last_modified_time)
    ...
    if current_modified_time != last_modified_time:
        ...
        if message:
            # Memorizza il messaggio e lo stampa sul terminale SOLO se è cambiato
            message_content = message
            print(f"Contenuto JSON aggiornato: {message_content}")
            
            new_fsa = parse_fsa_message(message)
            if new_fsa is None:
                print("Errore: Parsing del messaggio FSA fallito")
                exit(1)  # Termina il programma con codice di errore
            else:
                current_fsa = new_fsa
                FSA = new_fsa  # Update the global FSA
        else:
            print("Errore: File FSA non trovato o non leggibile")
            exit(1)  # Termina il programma con codice di errore
    ...
    return current_fsa, last_modified_time
